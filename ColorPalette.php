...
class ColorPalette {
	 ...
	 */
	static private
	function CacheFromUrl($imageUrl, $useCache = true) {
		$localFile = sys_get_temp_dir().
		"/".md5($imageUrl);
	 ...
	 */
	static public
	function GenerateFromUrl($imageUrl, $useCache = true) {
	 ...
	 */
	static public
	function GenerateFromUrls($imageUrlList, $useCache = true) {
	 ...
	 */
	static public
	function GenerateFromLocalImage($filePath, $is_data_uri = false) {
		if (($is_data_uri != 1) && !file_exists($filePath)) {
		...
		$PREVIEW_HEIGHT = 150;
		if ($is_data_uri == 1) {
			$size = getimagesizefromstring($filePath);
		} else {
		...
		if ($is_data_uri == 1) {
			$image_orig = imagecreatefromstring($filePath);
		} else {
